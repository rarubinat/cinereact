...
const Payment = () => {
  ...
  const formatExpiry = (value) => value.replace(/\D/g, "").replace(/(\d{2})(\d{1,2})?/, (_, m1, m2) => m2 ? `${m1}/${m2}` : m1);

  const handlePayment = () => {
    if (!name || !cardNumber || !expiry || !cvv) return showAlert("Please fill in all payment details.");
    if (cardNumber.replace(/\s/g, "").length !== 16) return showAlert("Card number must be 16 digits.");
    if (!/^\d{2}\/\d{2}$/.test(expiry)) return showAlert("Expiry must be in MM/YY format.");
    if (cvv.length < 3 || cvv.length > 4) return showAlert("CVV must be 3 or 4 digits.");
    ...
    setTimeout(() => {
      db.collection("reservas")
        ...
        .then(() => {
          ...
          setPaymentCompleted(true); // <-- cambia el estado para mostrar la confirmación
        })
        .catch((error) => {
          setLoading(false);
  ...
  if (paymentCompleted && reservationData) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-green-50 text-gray-900 p-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md w-full max-w-md space-y-2">
          ...
          <p className="mt-2 text-xl font-bold">Total: {reservationData.totalPrice.toFixed(2)} €</p>
        </div>
